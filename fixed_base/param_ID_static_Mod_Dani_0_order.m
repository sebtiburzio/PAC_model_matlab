...
fun = @FitShapeGivenE;
E = fminsearch(fun,[k_obj,Theta_bar(1)],options);
...
for i = 1:length(Gamma_set)
    ...
    for i_s = 1:length(s_vect)
        xy_c(i_s,:) = fk_fcn1(p_vals, Theta_set(:,i), s_vect(i_s), 0);
    end
    
    Gamma_here2 = -Gamma_set(i)*1;
    ...
    Pos_mid_set2 = xy_c(ind_middle,:);
    Pos_mid_set2_all = [Pos_mid_set2_all Pos_mid_set2'];
    Pos_end_set2 = xy_c(end,:);
    Pos_end_set2_all = [Pos_end_set2_all Pos_end_set2'];

    % hold on;plot(Pos_mid_set2(1),Pos_mid_set2(2),'o');
    % hold on;plot(Pos_end_set2(1),Pos_end_set2(2),'o')
    %%%%%%%%%%%%%%%%%%%%%%%%%
end


Pos_mid_set = Pos_mid_set2_all;
Pos_end_set = Pos_end_set2_all;
...
% Optimum but we do not use because it is far away for the other values
% k_obj = 0.0805;
% theta_bar =[0.2572];
...
k = [];
    ...
    % for i = 14:2:16
        ...
        Theta_set_here   = Theta_set(1:2,i);        % I USE THIS AS AN INITIAL GUESS
        ...
        if mod(i,2)==1
            LB = [ones(length(ini_guess2),1)*0];
        else          
            UB = [ones(length(ini_guess2),1)*0];
        end
        ...
        xy_c = nan(length(s_vect),2);
        for i_s = 1:length(s_vect)
            xy_c(i_s,:) = fk_fcn(p_vals, [z(1)]', s_vect(i_s), 0);
        end
