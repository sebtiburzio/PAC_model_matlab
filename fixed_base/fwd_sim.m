...
x_0 = [1.9557371360428932; -3.0235858830099405];
dx_0 = [0; 0];
...
[~, ss_range] = min(abs(Gamma-[pi/2 pi/4 0 -pi/4 -pi/2]));
for i = ss_range
    G_dir = -Gamma(i); % Note- use -ve Gamma since since data is robot angle
    out = sim('ss_solver');
    q_ev = out.q_ev;
    plot_config(Theta0(i),Theta1(i), 0.5, 0) % Gamma(i))
    hold on
    plot_config(q_ev.Data(end,1), q_ev.Data(end,2), 1, 0) % Gamma(i))
    hold on
